...
const ViewPage = ({ params: { locale } }: { params: LocalesProps }) => {
    ...
    const removeEmploy = createEmployeeStore((state: any) => state?.removeEmployee)
                        ...
                        <div key={item.id} className="p-3 flex flex-col border-black border-2 items-center rounded-md mx-5 my-5 h-80 w-52 justify-around">
                            ...
                            <div className="p-1 flex flex-row gap-2 items-center">
                                <Link href={`/edit/${item.id}`}>
                                    <button className="text-sm justify-center text-black border-2 border-black rounded-md transition hover:bg-gradient-to-r from-green-500 to-green-700 hover:text-white">
